package com.github.mobile.mock.httpservice;
...
public class HttpCaseHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
	private static final String MOCK_URL = "/github/service/mock";
	...
	protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) throws Exception {		
		...
		for(HttpCaseInfo tmp : queue){
			...
			if(url.equals(info.getUrl())){
				String cmdCode = tmp.getCmdcode();
				
				//如果队列中的已有的用例的URL与code相同，则直接覆盖对列中已有的
				if(cmdCode.equals(info.getCmdcode())){
					queue.remove(tmp);
					//break;					
				}				
			}
